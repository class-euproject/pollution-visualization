...
menu_selector <- c("LinkID", "Speed","NOx", "HC","CO", "PM" ,"PN" ,"NO")
  ...
  sidebarLayout(position = "right",
    sidebarPanel(
      selectInput("gasType", "Gas type",
                  choices = c("All", menu_selector[1:8]),
                  selected = "All"
      ),
      width = 2
...
server <- function(input, output) {
  ...
  zcolumn <- reactive({
    value <- NULL
    if(input$gasType != "All")  value <- input$gasType
    value
  })
  ...
  output$mapPlot <- renderLeaflet({
    ...
    zcolValue <- zcolumn()
    #generate a map
    mapviewOptions(basemaps = c("OpenStreetMap"), vector.palette =  colorRampPalette(c("green","yellow","red","black")))
